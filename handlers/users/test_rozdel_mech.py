...
class Form_mech(StatesGroup):
    otv1_mech = State()
    otv2_mech = State()
    otv3_mech = State()

@router.message(F.text == 'Тест по теме "Механика"')
...
@router.message(Form_mech.otv1_mech)
async def get_fio(message: Message, state: FSMContext):
    id_user = message.chat.id
    cursor.execute("SELECT otvet FROM test_mech")
    cor_otv = cursor.fetchall()
    cursor.execute("SELECT foto FROM test_mech")
