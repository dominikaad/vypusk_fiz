...
class Form_el(StatesGroup):
    otv1_elekdyn = State()
    otv2_elekdyn = State()
    otv3_elekdyn = State()

@router.message(F.text == 'Тест по теме "Электродинамика"')
async def test_mech(message: Message, state: FSMContext):
    cursor.execute("SELECT number FROM test_elekdyn")
    num = cursor.fetchall()
    cursor.execute("SELECT uslov FROM test_elekdyn")
    ...
    await state.set_state(Form_el.otv1_elekdyn)
    await message.answer_photo(photo=task_image1, reply_markup=types.ReplyKeyboardRemove())

@router.message(Form_el.otv1_elekdyn)
async def get_fio(message: Message, state: FSMContext):
    id_user = message.chat.id
    cursor.execute("SELECT otvet FROM test_elekdyn")
    cor_otv = cursor.fetchall()
    cursor.execute("SELECT foto FROM test_elekdyn")
    ...
    try:
        ...
        task_image1 = FSInputFile(fot1)
        await state.set_state(Form_el.otv2_elekdyn)
        await message.answer_photo(photo=task_image1, reply_markup=types.ReplyKeyboardRemove())
    except ValueError:
...
async def get_fio(message: Message, state: FSMContext):
    ...
    try:
        ...
        task_image1 = FSInputFile(fot1)
        await state.set_state(Form_el.otv3_elekdyn)
        await message.answer_photo(photo=task_image1, reply_markup=types.ReplyKeyboardRemove())
    except ValueError:
        await message.answer(text='Округлите число до целого')
        return
