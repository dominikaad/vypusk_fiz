...
class Form_op(StatesGroup):
    otv1_optyka = State()
    otv2_optyka = State()
    otv3_optyka = State()

@router.message(F.text == 'Тест по теме "Оптика"')
async def test_mech(message: Message, state: FSMContext):
    cursor.execute("SELECT number FROM test_optyka")
    num = cursor.fetchall()
    cursor.execute("SELECT uslov FROM test_optyka")
    ...
    await state.set_state(Form_op.otv1_optyka)
    await message.answer_photo(photo=task_image1, reply_markup=types.ReplyKeyboardRemove())

@router.message(Form_op.otv1_optyka)
async def get_fio(message: Message, state: FSMContext):
    id_user = message.chat.id
    cursor.execute("SELECT otvet FROM test_optyka")
    cor_otv = cursor.fetchall()
    cursor.execute("SELECT foto FROM test_optyka")
    ...
    task_image1 = FSInputFile(fot1)
    await state.set_state(Form_op.otv2_optyka)
    await message.answer_photo(photo=task_image1, reply_markup=types.ReplyKeyboardRemove())

@router.message(Form_op.otv2_optyka)
async def get_fio(message: Message, state: FSMContext):
    id_user = message.chat.id
    cursor.execute("SELECT otvet FROM test_optyka")
    cor_otv = cursor.fetchall()
    ...
    task_image1 = FSInputFile(fot1)
    await state.set_state(Form_op.otv3_optyka)
    await message.answer_photo(photo=task_image1, reply_markup=types.ReplyKeyboardRemove())

@router.message(Form_op.otv3_optyka)
async def get_fio(message: Message, state: FSMContext):
    id_user = message.chat.id
    cursor.execute("SELECT otvet FROM test_optyka")
    cor_otv = cursor.fetchall()
