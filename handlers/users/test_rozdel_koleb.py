...
class Form_kol(StatesGroup):
    otv1_koleb = State()
    otv2_koleb = State()
    otv3_koleb = State()

@router.message(F.text == 'Тест по теме "Колебания и волны"')
async def test_mech(message: Message, state: FSMContext):
    cursor.execute("SELECT number FROM test_koleb")
    num = cursor.fetchall()
    cursor.execute("SELECT uslov FROM test_koleb")
    ...
    await state.set_state(Form_kol.otv1_koleb)
    await message.answer_photo(photo=task_image1, reply_markup=types.ReplyKeyboardRemove())

@router.message(Form_kol.otv1_koleb)
async def get_fio(message: Message, state: FSMContext):
    id_user = message.chat.id
    cursor.execute("SELECT otvet FROM test_koleb")
    cor_otv = cursor.fetchall()
    cursor.execute("SELECT foto FROM test_koleb")
    ...
    try:
        ...
        task_image1 = FSInputFile(fot1)
        await state.set_state(Form_kol.otv2_koleb)
        await message.answer_photo(photo=task_image1, reply_markup=types.ReplyKeyboardRemove())
    except ValueError:
        await message.answer(text='Округлите число до целого')
        return

@router.message(Form_kol.otv2_koleb)
async def get_fio(message: Message, state: FSMContext):
    id_user = message.chat.id
    cursor.execute("SELECT otvet FROM test_koleb")
    cor_otv = cursor.fetchall()
    ...
    try:
        ...
        task_image1 = FSInputFile(fot1)
        await state.set_state(Form_kol.otv3_koleb)
        await message.answer_photo(photo=task_image1, reply_markup=types.ReplyKeyboardRemove())
    except ValueError:
        await message.answer(text='Округлите число до целого')
        return

@router.message(Form_kol.otv3_koleb)
async def get_fio(message: Message, state: FSMContext):
    id_user = message.chat.id
    cursor.execute("SELECT otvet FROM test_koleb")
    cor_otv = cursor.fetchall()
